...
def predict_api():
    ...
    required_fields = ["Impressions", "Clicks", "Campaign_Type", "Region", "Device_Type", "Month", "Conversion_Rate", "CPM"]
    for field in required_fields:
        if field not in data:
            return jsonify({"error": f"Missing field: {field}"}), 400

    campaign_map = {"Display": 0, "Search": 1, "Social": 2}
    region_map = {"Canada": 0, "USA": 1, "UK": 2}
    device_map = {"Desktop": 0, "Mobile": 1, "Tablet": 2}

    data["Campaign_Type"] = campaign_map.get(data["Campaign_Type"], 0)
    data["Region"] = region_map.get(data["Region"], 0)
    data["Device_Type"] = device_map.get(data["Device_Type"], 0)
